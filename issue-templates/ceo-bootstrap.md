...
각 child 완료 시 **사람 사용자의 명시적 코멘트 승인**을 게이트로 둔다 (아래 "사용자 승인 게이트" 섹션 참조).

---

# 조직 / 프로젝트 ID 맵 (child issue 생성 시 참조)

> **CEO 가 본문을 받자마자 가장 먼저 할 일** — 아래 8개 `{{...}}` placeholder 를 본인 회사의 실제 UUID 로 치환한다. 시청자가 손으로 바꾸지 않는다.
>
> 1. `GET /api/companies/{cid}/agents` 를 호출해 `role` 이 `researcher`·`analyst`·`engineer`·`ceo` 인 4명의 `id` 를 받는다.
> 2. `GET /api/companies/{cid}/projects` 를 호출해 `name` 이 `Onboarding`·`브랜드 리서치 시스템`·`분석 + SWOT + 팩트 체크`·`슬라이드 빌더` 인 4개의 `id` 를 받는다.
> 3. 본문 안의 8개 placeholder 를 해당 UUID 로 치환한 다음, 그 치환된 본문으로 child 이슈를 만들기 시작한다.
> 4. 회사·에이전트·프로젝트 이름이 정확히 일치하지 않으면 사용자에게 코멘트로 "ID 매핑 불일치" 를 보고하고 멈춘다.

| 역할 | name | placeholder |
|---|---|---|
| 브랜드 리서처 | researcher | `{{BRAND_RESEARCHER_AGENT_ID}}` |
| 데이터 분석가 | analyst (general) | `{{DATA_ANALYST_AGENT_ID}}` |
| 슬라이드 제작자 | engineer | `{{SLIDE_BUILDER_AGENT_ID}}` |
| CEO | ceo (self) | `{{CEO_AGENT_ID}}` |

| project name | placeholder |
|---|---|
| Onboarding (이 부트스트랩 이슈 자체) | `{{ONBOARDING_PROJECT_ID}}` |
| 브랜드 리서치 시스템 (단계 1) | `{{BRAND_RESEARCH_PROJECT_ID}}` |
| 분석 + SWOT + 팩트 체크 (단계 2) | `{{ANALYSIS_SWOT_PROJECT_ID}}` |
| 슬라이드 빌더 (단계 3, 4) | `{{SLIDE_BUILDER_PROJECT_ID}}` |

---

# 진행 흐름 (4단계 child issue 자동 위임)

각 단계는 직전 단계가 사람 사용자의 승인까지 받은 다음에만 시작한다 (`blockedByIssueIds` + 사용자 승인 코멘트 둘 다 충족).

## 단계 1 — 브랜드 리서처에게 위임

- **assigneeAgentId**: `{{BRAND_RESEARCHER_AGENT_ID}}` (브랜드 리서처)
- **projectId**: `{{BRAND_RESEARCH_PROJECT_ID}}` (브랜드 리서치 시스템)
...
- **assigneeAgentId**: `{{DATA_ANALYST_AGENT_ID}}` (데이터 분석가)
- **projectId**: `{{ANALYSIS_SWOT_PROJECT_ID}}` (분석 + SWOT + 팩트 체크)
- **parentId**: 이 부트스트랩 issue 의 id
...
- **assigneeAgentId**: `{{SLIDE_BUILDER_AGENT_ID}}` (슬라이드 제작자)
- **projectId**: `{{SLIDE_BUILDER_PROJECT_ID}}` (슬라이드 빌더)
- **parentId**: 이 부트스트랩 issue 의 id
...
Vercel 에 배포합니다. 정적 PDF 가 아니라 사용자가 슬랙·이메일로 공유 가능한 URL 을 만듭니다.
...
`VERCEL_API_TOKEN` 이 비어 있으면 즉시 사용자에게 코멘트로 알리고 진행 중단.
...
- **assigneeAgentId**: `{{CEO_AGENT_ID}}` (CEO self)
- **projectId**: `{{SLIDE_BUILDER_PROJECT_ID}}` (슬라이드 빌더 — 빌드 결과물 위에서 이루어짐)
- **parentId**: 이 부트스트랩 issue 의 id
...
각 child issue 가 done 으로 넘어가기 전에 **본 부트스트랩 이슈 (parent) 에 다음 형식으로 코멘트를 작성**한 다음, **사람 사용자의 명시적 코멘트 응답 ("승인" 또는 "Approve")** 을 받은 후에만 다음 child 단계를 위임한다.
...
- 산출물: <파일/링크 리스트>
- 핵심 결과: <2~3줄 요약>
- 검토 포인트: <사용자가 확인해야 할 항목>
- 다음 단계 예고: <위임할 child 의 담당자와 임무>

@dante 위 결과 검토 후 "승인" 또는 "반려: <이유>" 코멘트 부탁드립니다.
```

사용자가 "**반려: <이유>**" 로 응답하면 **수정 child issue 를 새로 만들어** 그 단계를 다시 진행한다 (원본 child 는 in_review 로 두고 수정본 done 후 묶어서 다시 보고).
...
- **회사 도메인 스킬** — brand-research-glossary, swot-from-signals, nextjs-tremor-report, report-evidence-citation

# 절대 제약 (회사 전역)

- **새 에이전트 채용 금지**. 3명 직원 + CEO 의 4인 구성 고정. paperclip-create-agent 스킬 호출 금지.
- **사용자 승인 게이트 우회 금지**. 각 child done 전 반드시 사용자 코멘트 응답 확인.
